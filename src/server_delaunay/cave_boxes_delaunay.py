...
# Step 2: Compute the Delaunay tetrahedralization of the point cloud
delaunay = Delaunay(points)
tetrahedrons = delaunay.simplices
print(f"Number of tetrahedrons: {len(tetrahedrons)}")
# Step 3: Extract the surface triangles from the tetrahedra
def extract_surface_triangles(tetrahedrons):
    from collections import defaultdict
    
    # Dictionary to keep track of triangle faces
    face_counts = defaultdict(int)
    
    # Iterate over tetrahedrons to extract faces
    ...
    # Only keep faces that belong to a single tetrahedron
...
surface_triangles = extract_surface_triangles(tetrahedrons)

# Convert to trimesh format
trimesh_mesh = trimesh.Trimesh(vertices=points, faces=surface_triangles)

# # Optionally, save or visualize the mesh
# o3d_trimesh = o3d.geometry.TriangleMesh(
#     vertices=o3d.utility.Vector3dVector(trimesh_mesh.vertices),
#     triangles=o3d.utility.Vector3iVector(trimesh_mesh.faces)
# )

# o3d.io.write_triangle_mesh("delaunay_mesh.ply", o3d_trimesh)
# o3d.visualization.draw_geometries([o3d_trimesh])

# Step 4: Convert the mesh to a voxel grid using trimesh
# Define voxel size
...
with open(f'boxes_{int(1/scale)}_delaunay.csv', mode='w', newline='') as file:
...
print(f"Boxes have been exported to 'boxes_{int(1/scale)}_delaunay.csv'")


def write_vtk_file(boxes_real, filename=f'boxes{int(1/scale)}_delaunay.vtk'):
...
write_vtk_file(boxes_real, f'boxes_{int(1/scale)}_delaunay.vtk')
